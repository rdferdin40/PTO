<div class="mb-4">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active"><?= e($editUser['first_name'] . ' ' . $editUser['last_name']) ?></li>
        </ol>
    </nav>
</div>
...
<div class="row">
    <div class="col-md-8">
        <div class="card">
            <div class="card-body">
                <form method="POST" action="<?= $app->url('/admin/users/' . $editUser['id']) ?>">
                    <?= csrf_field() ?>

                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="first_name" class="form-label"><?= __('first_name') ?> <span class="text-danger">*</span></label>
                            <input type="text" name="first_name" id="first_name" class="form-control" value="<?= e($editUser['first_name']) ?>" required>
                        </div>

                        <div class="col-md-6 mb-3">
                            <label for="last_name" class="form-label"><?= __('last_name') ?> <span class="text-danger">*</span></label>
                            <input type="text" name="last_name" id="last_name" class="form-control" value="<?= e($editUser['last_name']) ?>" required>
                        </div>
                    </div>

                    <div class="mb-3">
                        <label for="email" class="form-label"><?= __('email') ?> <span class="text-danger">*</span></label>
                        <input type="email" name="email" id="email" class="form-control" value="<?= e($editUser['email']) ?>" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="role" id="role" class="form-select" required>
                            <option value="employee" <?= $editUser['role'] == 'employee' ? 'selected' : '' ?>><?= __('role_employee') ?></option>
                            <option value="manager" <?= $editUser['role'] == 'manager' ? 'selected' : '' ?>><?= __('role_manager') ?></option>
                            <option value="admin" <?= $editUser['role'] == 'admin' ? 'selected' : '' ?>><?= __('role_admin') ?></option>
                        </select>
                    </div>

                    <div class="mb-3 form-check">
                        <input type="checkbox" name="is_manager" id="is_manager" class="form-check-input" <?= $editUser['is_manager'] ? 'checked' : '' ?>>
                    ...
                    <div class="mb-3 form-check">
                        ...
                        <label for="is_active" class="form-check-label"><?= __('is_active') ?></label>
                        <small class="form-text text-muted d-block"><?= __('is_active_help') ?></small>
                    </div>
